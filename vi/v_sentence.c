...
v_sentencef(SCR *sp, VICMD *vp)
{
	 ...
	 */
	if (cs.cs_flags == CS_EMP || (cs.cs_flags == 0 && issptab(cs.cs_ch))) {
		if (cs_fblank(sp, &cs))
			return (1);
		if (--cnt == 0) {
		...
		if (cs.cs_flags == CS_EOL) {
			if ((state == PERIOD || state == BLANK) && --cnt == 0) {
				if (cs_next(sp, &cs))
					return (1);
				if (cs.cs_flags == 0 &&
				    issptab(cs.cs_ch) && cs_fblank(sp, &cs))
					return (1);
				goto okret;
			}
			state = NONE;
...
v_sentenceb(SCR *sp, VICMD *vp)
{
	...
	} else if (cs.cs_flags == 0 && !issptab(cs.cs_ch))
		for (;;) {
			if (cs_prev(sp, &cs))
				return (1);
			if (cs.cs_flags != 0 || issptab(cs.cs_ch))
				break;
		}
...
ret:			slno = cs.cs_lno;
			...
			do {
			    ...
			    (cs.cs_ch == ')' || cs.cs_ch == ']' ||
			    cs.cs_ch == '"' || cs.cs_ch == '\''));
			if ((cs.cs_flags || issptab(cs.cs_ch)) &&
			    cs_fblank(sp, &cs))
				return (1);
		...
		default:
			last =
			    cs.cs_flags == CS_EOL || issptab(cs.cs_ch) ||
			    cs.cs_ch == ')' || cs.cs_ch == ']' ||
